...
void j1Map::PropagateBFS()
{
	...
	if (frontier.start != nullptr)
	{
		...
		for (int i = 0; i < 4; ++i)
		{
			tile_neighbour[i] = tile;
		}

		int i = 0;
		tile_neighbour[i++] += {0,1};
		tile_neighbour[i++] += {0, -1};
		tile_neighbour[i++] += {1, 0};
		tile_neighbour[i++] += {-1, 0};
	...
	for (int i = 0; i < 4; i++)
	{
		if (visited.find(tile_neighbour[i]) == -1 && IsWalkable(tile_neighbour[i].x, tile_neighbour[i].y))
		{
			frontier.Push(tile_neighbour[i]);
			visited.add(tile_neighbour[i]);
		}
	}
}
